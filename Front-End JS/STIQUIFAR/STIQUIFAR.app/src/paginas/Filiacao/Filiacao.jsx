...
function Filiacao() {
  ...
  async function buscarEnderecoPorCep(cepInformado) {
    const cepLimpo = somenteNumeros(cepInformado);

    if (cepLimpo.length !== 8) {
      return;
    }

    if (cepConsultadoRef.current === cepLimpo) {
      return;
    }
    ...
    setMensagem('Buscando endereço pelo CEP...');
    setTipoMensagem('info');

    const resultadoCep = await verificarCepExistente(cepLimpo);
    ...
    const dados = resultadoCep.dados;

    setForm((formAtual) => {
      if (somenteNumeros(formAtual.cep) !== cepLimpo) {
        return formAtual;
      }
      ...
      return {
        ...
        endereco: dados.logradouro || formAtual.endereco,
        bairro: dados.bairro || formAtual.bairro,
        cidade: dados.localidade || formAtual.cidade
      };
    });
  ...
  function alterarForm(evento) {
    ...
    if (name === 'cep') {
      ...
      const cepLimpo = somenteNumeros(valorFinal);
      ...
      if (cepLimpo.length === 8) {
        buscarEnderecoPorCep(valorFinal);
      }
  ...
  async function enviarFormulario(evento) {
    ...
    try {
      ...
      const resultado = await Swal.fire({
        ...
        html: `
          <strong>Solicitação de filiação enviada com sucesso.</strong>
          <br />
          <span>Em breve o STIQUIFAR entrará em contato para dar continuidade ao processo.</span>
        `,
        confirmButtonText: 'Voltar para a landing page',
        draggable: true
      });
      ...
      Swal.fire({
        ...
        text: 'Não foi possível enviar a solicitação. Tente novamente em alguns instantes.',
      ...
      {carregandoCep && (
        <div className={styles.telaCarregamento}>
          <div className={styles.cardCarregamento}>
            ...
            <strong>Buscando endereço</strong>

            <p>
              Aguarde enquanto consultamos o CEP informado e preenchemos os dados
              automaticamente.
            </p>
          </div>
        </div>
            ...
            <div className={styles.grid}>
              ...
              <div className={`${styles.campo} ${styles.completo}`}>
                ...
                <input
                  ...
                  onChange={alterarForm}
                  placeholder="Apartamento, bloco, número, referência..."
                />
              </div>
